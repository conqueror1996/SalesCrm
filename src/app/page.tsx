...
export default function Dashboard() {
  ...
  const [currentView, setCurrentView] = useState<'dashboard' | 'crm' | 'catalog'>('crm');
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '2rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
            <div style={{ width: '32px', height: '32px', background: 'linear-gradient(135deg, var(--primary), #4f46e5)', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 'bold' }}>U</div>
            <h1 style={{ fontSize: '1.1rem', fontWeight: 700, letterSpacing: '-0.5px' }}>UrbanClay</h1>
          </div>

          <nav style={{ display: 'flex', gap: '0.5rem', background: 'rgba(255,255,255,0.03)', padding: '4px', borderRadius: '10px', border: '1px solid var(--glass-border)' }}>
            {[
              { id: 'dashboard', label: 'Dashboard', icon: '📊' },
              { id: 'crm', label: 'CRM / Chat', icon: '💬' },
              ...
              <button
                ...
                style={{
                  background: currentView === tab.id ? 'var(--surface-2)' : 'transparent',
      ...
      <div style={{ display: currentView === 'crm' ? 'flex' : 'none', flex: 1, overflow: 'hidden' }}>
        ...
        <div className={styles.sidebar} style={{
          ...
          <div className={styles.leadList}>
            {filteredLeads.map((lead) => {
              const gu = (lead as any).guidance as AIGuidance;
              return (
                <div
                  key={lead.id}
        ...
        <div className={styles.chatArea} style={{
          ...
          {viewMode === 'chat' && activeLead ? (
            !whatsappStatus.connected ? (
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: 'var(--text-secondary)', gap: '1.5rem', background: '#0f172a', padding: '2rem' }}>
                <div style={{ textAlign: 'center' }}>
                  <h2 style={{ color: 'white', marginBottom: '0.5rem' }}>WhatsApp Not Connected</h2>
                  <p style={{ maxWidth: '400px', margin: '0 auto', fontSize: '0.9rem' }}>Scan the QR code below with your WhatsApp mobile app to connect the CRM.</p>
                </div>

                <div style={{ padding: '2rem', background: 'white', borderRadius: '16px', minWidth: '340px', minHeight: '340px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', boxShadow: '0 10px 40px rgba(0,0,0,0.3)' }}>
                  {whatsappStatus.qr ? (
                    <>
                ...
                {[
                  ...
                  return (
                    <div key={col.title} style={{ width: '280px', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                      ...
                      {totalValue > 0 && (
                        <div style={{ fontSize: '0.7rem', color: 'var(--success)', fontWeight: 600 }}>₹{totalValue.toLocaleString()} Potentail</div>
                      )}
        ...
        <div className={styles.intelligencePanel} style={{
          ...
          <div className={styles.panelHeader} style={{ padding: '1rem 1.5rem' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <div style={{ fontSize: '0.65rem', color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Revenue Pipeline</div>
                <div style={{ fontSize: '1.2rem', fontWeight: 800, color: 'white', marginTop: '2px' }}>
                  ₹{(stats.totalRevenue / 100000).toFixed(1)}L
                </div>
              </div>
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: '0.65rem', color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Hot Leads</div>
              ...
              <div className="animate-fade-in">
                {(() => {
                  ...
                  return (
                    <>
                      ...
                      <div className={styles.infoCard}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem' }}>
                          <div>
                            ...
                            <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginTop: '4px' }}>
                              📍 {guidance.location || 'Location Pending'}
                            </div>
                          </div>
                          <div style={{
                            width: '12px', height: '12px', borderRadius: '50%',
                            background: guidance.qualificationStatus === '🟢 Got area + location' ? 'var(--success)' : (guidance.qualificationStatus === '🟡 Got project type' ? 'var(--warning)' : 'var(--error)'),
                            boxShadow: `0 0 12px ${guidance.qualificationStatus === '🟢 Got area + location' ? 'rgba(16, 185, 129, 0.4)' : 'rgba(239, 68, 68, 0.4)'}`
                          }} title={guidance.qualificationStatus} />
                        </div>
                        ...
                        <div className={styles.infoCard} style={{ padding: '0.75rem', marginBottom: '1rem' }}>
                          ...
                          <div style={{ height: '6px', background: 'rgba(255,255,255,0.1)', borderRadius: '3px', overflow: 'hidden' }}>
                            <div style={{
                              ...
                              background: (guidance.seriousBuyerScore || 0) > 75 ? 'var(--error)' : ((guidance.seriousBuyerScore || 0) > 40 ? 'var(--warning)' : '#3b82f6'),
                        ...
                        <div className={styles.infoCard}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                            ...
                            <span style={{
                              ...
                              padding: '2px 8px',
                              borderRadius: '4px',
                              background: guidance.decisionPressure === 'Fast Mover' ? 'rgba(16, 185, 129, 0.2)' : (guidance.decisionPressure === 'Dropping' ? 'rgba(239, 68, 68, 0.2)' : 'rgba(255,255,255,0.05)'),
                              color: guidance.decisionPressure === 'Fast Mover' ? 'var(--success)' : (guidance.decisionPressure === 'Dropping' ? 'var(--error)' : 'white')
                            }}>
                              {guidance.decisionPressure.toUpperCase()}
                            </span>
                          </div>

                          {/* OBJECTION DETECTOR */}
                          {guidance.objectionDetected && guidance.objectionDetected !== 'None' ? (
                            <div style={{ marginTop: '12px', padding: '8px', background: 'rgba(239, 68, 68, 0.15)', border: '1px solid #f87171', borderRadius: '8px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                              <span style={{ fontSize: '1.2rem' }}>🛡️</span>
                              <div>
                                <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#f87171' }}>OBJECTION: {guidance.objectionDetected.toUpperCase()}</div>
                                <div style={{ fontSize: '0.7rem', color: '#fca5a5' }}>
                                  {guidance.objectionDetected === 'Price' ? 'User is negotiating rate.' :
                                    guidance.objectionDetected === 'Delay' ? 'User is delaying decision.' :
                                      'Competitor comparison detected.'}
                                </div>
                              </div>
                            </div>
                          ) : (
                            <div style={{ marginTop: '8px', fontSize: '0.75rem', color: 'var(--text-secondary)', fontStyle: 'italic', display: 'flex', alignItems: 'center', gap: '6px' }}>
                              <span>✅</span> No objections detected yet.
                            </div>
                          )}
                      ...
                      {/* BRUTAL ALERT FOR GHOSTING */}
                      {
                        (guidance.ghostingProbability || 0) > 60 && (
                          <div className="glass-panel" style={{ padding: '0.8rem', borderRadius: '8px', border: '1px solid var(--error)', background: 'rgba(239, 68, 68, 0.1)', marginBottom: '1.5rem', animation: 'pulse 2s infinite' }}>
                            <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                              <div style={{ fontSize: '1.5rem' }}>🚑</div>
                              <div>
                                <div style={{ fontSize: '0.8rem', fontWeight: 900, color: 'var(--error)', textTransform: 'uppercase' }}>RESCUE MODE ACTIVATED</div>
                                <div style={{ fontSize: '0.7rem', color: '#fca5a5' }}>Ghosting detected. Use the specialized rescue scripts below to re-engage the client.</div>
                              </div>
                            </div>
                          </div>
                        )
                      ...
                      {/* CLOSURE INTELLIGENCE */}
                      {
                        guidance.objectionIntensity && (
                          <div className="glass-panel" style={{ padding: '0.8rem 1.2rem', borderRadius: 'var(--radius-md)', marginBottom: '1.5rem', background: 'rgba(255,255,255,0.02)', border: '1px solid var(--glass-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      {/* MANUAL SERIOUSNESS LOGGER */}
                      <div className="glass-panel" style={{ padding: '0.8rem', borderRadius: '8px', background: 'rgba(255,255,255,0.02)', border: '1px solid var(--glass-border)', marginBottom: '1.5rem' }}>
                        <div style={{ fontSize: '0.7rem', color: 'var(--text-secondary)', marginBottom: '8px', textTransform: 'uppercase' }}>Log Lead Intent (Seriousness Test)</div>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px' }}>
                          <button onClick={() => alert('Logged: Villa Project (High Value)')} style={{ background: 'rgba(16, 185, 129, 0.1)', border: '1px solid var(--success)', color: 'var(--success)', padding: '6px', borderRadius: '4px', fontSize: '0.7rem', cursor: 'pointer' }}>Villa Project</button>
                          <button onClick={() => alert('Logged: Renovation (Medium Value)')} style={{ background: 'rgba(245, 158, 11, 0.1)', border: '1px solid var(--warning)', color: 'var(--warning)', padding: '6px', borderRadius: '4px', fontSize: '0.7rem', cursor: 'pointer' }}>Renovation</button>
                          <button onClick={() => alert('Logged: Boundary Wall (Low Value)')} style={{ background: 'rgba(99, 102, 241, 0.1)', border: '1px solid var(--primary)', color: 'var(--primary)', padding: '6px', borderRadius: '4px', fontSize: '0.7rem', cursor: 'pointer' }}>Boundary Wall</button>
                          <button onClick={() => alert('Logged: Rate Checking (Low Value)')} style={{ background: 'rgba(239, 68, 68, 0.1)', border: '1px solid var(--error)', color: 'var(--error)', padding: '6px', borderRadius: '4px', fontSize: '0.7rem', cursor: 'pointer' }}>Just Checking</button>
                        </div>
                      </div>
                      ...
                      {/* CLIENT BEHAVIOR */}
                      {
                        guidance.clientBehavior && (
                          <div className="glass-panel" style={{ padding: '1rem', borderRadius: 'var(--radius-md)', marginBottom: '1.5rem', border: '1px dashed var(--glass-border)', background: 'transparent' }}>
                            <div style={{ fontSize: '0.65rem', color: 'var(--text-tertiary)', textTransform: 'uppercase', marginBottom: '8px', letterSpacing: '0.5px' }}>Client Behavior Profile</div>
                            <div style={{ display: 'flex', gap: '2rem' }}>
                              <div>
                                <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>Responsiveness</div>
                                <div style={{ fontSize: '0.85rem', fontWeight: 600, color: guidance.clientBehavior.responsiveness === 'Fast' ? 'var(--success)' : 'white' }}>{guidance.clientBehavior.responsiveness}</div>
                              </div>
                              <div>
                                <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>Seriousness</div>
                                <div style={{ fontSize: '0.85rem', fontWeight: 600, color: guidance.clientBehavior.seriousnessScore === 'High' ? 'var(--primary)' : 'white' }}>{guidance.clientBehavior.seriousnessScore}</div>
                              </div>
                              <div>
                                <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>Tone</div>
                                <div style={{ fontSize: '0.85rem', fontWeight: 600 }}>{guidance.clientBehavior.tone}</div>
                              </div>
                            </div>
                          </div>
                        )
                ...
                <div style={{ marginTop: '2rem', flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
                  ...
                  <div style={{ display: 'flex', gap: '1rem', height: '500px' }}>
                    ...
                    <div className="glass-panel" style={{ width: '140px', display: 'flex', flexDirection: 'column', gap: '6px', overflowY: 'auto', padding: '12px', background: 'rgba(255,255,255,0.02)', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.05)' }}>
